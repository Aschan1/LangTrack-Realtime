...
#from ultralytics.ultralytics.engine import model#Comment as model loaded later
...
def inference(json_path, images_dir, conf_thresh=0.01, iou_thresh=0.5, validation=False):
    ...
    #yolo_model = YOLOE("ultralytics/yoloe-26s-seg.pt")
    #yolo_model.to(device)

    #print("Loading SigLIP...")
    #siglip_processor = AutoProcessor.from_pretrained("IDEA-Research/grounding-dino-tiny")
    ...
    for img_id, anns in tqdm(img_to_anns.items(), desc="Evaluating"):
        ...
        pred_text_labels = pred["text_labels"]

        for box, phrase in zip(pred["boxes"], pred_text_labels):
    ...
    # recall = matched_targets / total_targets if total_targets > 0 else 0
        ...
        for ann in anns:
            ...
            match_found = False
            for i, (p_box, p_label) in enumerate(zip(filtered_pred_boxes, pred_text_labels)):
